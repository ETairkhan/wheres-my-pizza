...
func (s *Server) Run() error {
	...
	s.mu.Lock()
	s.srv = &http.Server{
		Addr:    fmt.Sprintf(":%d", s.trackingParams.Port),
		Handler: s.mux,
	}
	s.mu.Unlock()

	mylog = mylog.WithGroup("details").With("port", s.trackingParams.Port)
...
func (s *Server) Configure() {
	...
	orderRepo := database.NewOrderRepo(s.ctx, s.db)
	workerRepo := database.NewWorkerRepo(s.ctx, s.db)

	orderService := services.NewOrderService(s.ctx, orderRepo, s.mylog)
	workerService := services.NewWorkerService(s.ctx, workerRepo, s.mylog)

	orderHandler := handle.NewOrderHandler(orderService, s.mylog)
	workerHandler := handle.NewWorkerHandler(workerService, s.mylog)
	// Register routes
	s.mux.Handle("GET /orders/{order_number}/status", orderHandler.GetStatus())
	s.mux.Handle("GET /orders/{order_number}/history", orderHandler.GetHistory())
	s.mux.Handle("GET /workers/status", workerHandler.GetStatus())
}
